...
model1=smf.ols("Sal~YE",data=Sl_d).fit()
...
plt.scatter(x=Sl_d['YE'],y=Sl_d['Sal'],color='red');plt.plot(Sl_d['YE'],pred1,color='black');plt.xlabel('YE');plt.ylabel('Sal')


## Transforming variables for accuracy ##
model2 = smf.ols('Sal~np.log(YE)',data=Sl_d).fit()
...
## Quadratic model ##
Sl_d["YE_Sq"] = Sl_d.YE*Sl_d.YE
model_quad = smf.ols("Sal~YE+YE_Sq",data=Sl_d).fit()
...
rmse = sqrt(mean_squared_error(Sl_d.Sal,pred_quad))
