...
url <- "https://twitter.com/narendramodi/status/1346405414267736065"
tweets_PMO <- NULL
for (i in 1:10){
  ...
  rev <- murl %>%
    ...
    html_text()
  tweets_PMO <- c(tweets_PMO,rev)
}
write.table(tweets_PMO,"tweets.txt",row.names = F)
...
plot(poa_sent,type = "h", main = "Tweets using transformed Values",
...
plot(ft_values, type = "h", main = "Tweets using Transformed values",
     xlab = "Narrative time", ylab = "Emotional Valence",
     col="red")
